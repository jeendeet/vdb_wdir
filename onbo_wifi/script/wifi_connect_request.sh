...
rm /data/vbd_workdir/log/temp.conf
sleep 2
...
wpa_supplicant -D nl80211 -i wlan0 -c /etc/wpa_supplicant.conf >> /data/vbd_workdir/log/temp.conf &
...
do
    ...
    echo $a
    a=`expr $a + 1`
    if grep -q "reason=WRONG_KEY" /data/vbd_workdir/log/temp.conf; then
        killall wpa_supplicant
        echo "WRONG_KEY"
        ...
        a=0
        break
    elif grep -q "CTRL-EVENT-CONNECTED" /data/vbd_workdir/log/temp.conf; then
        echo "CONNECTED"
